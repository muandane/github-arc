resource "azurerm_resource_group" "kp_cluster_rg" {
...
resource "azurerm_kubernetes_cluster" "kp_cluster" {
  name                      = format("%s-aks", replace(var.resource_group_name, "-rg", ""))
  location                  = azurerm_resource_group.kp_cluster_rg.location
  resource_group_name       = azurerm_resource_group.kp_cluster_rg.name
